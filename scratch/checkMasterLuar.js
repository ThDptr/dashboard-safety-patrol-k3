...
async function run() {
  ...
  const targetTopik = ["Luar Gedung", "APAR Luar", "APAR", "APAR (Luar)"];
  
  const luar = rows.filter(r => {
     if (!r.Topik && !r.Pertanyaan) return false;
     const topik = (r.Topik || "").toLowerCase();
     const pert = (r.Pertanyaan || "").toLowerCase();
     return topik.includes("luar") || pert.includes("luar") || pert.includes("rambu dan sop");
  });
