...
// 3. REMOVE perawat filter from APD isProfesi: false
// The original map block:
const perawatBlockStr = `      // Filter tags: ONLY "perawat" violations reduce the physical room's compliance
      top10 = top10.map(p => {
        let pTags = p.tags ? [...p.tags] : [];
        pTags = pTags.filter(t => t.toLowerCase().includes("perawat"));
        return { ...p, tags: pTags };
      });`;
if (code.includes(perawatBlockStr)) {
  code = code.replace(perawatBlockStr, `      // (Filter "perawat" dilepas, sekarang semua profesi mengurangi kepatuhan ruangan)`);
}

// 4. APD isProfesi: false photos
code = code.replace(
  /return \{\n\s*ruangan,\n\s*jumlahKaryawan,\n\s*tanggal,\n\s*namaPetugas,\n\s*keterangan,\n\s*patrols: top10,\n\s*isProfesi: false\n\s*\};/g,
  `const photos = top10
        .map((p, idx) => {
...
// 6. Fix table B match logic
const tableBLogicStr = `const matchCount = (p.tags || []).filter((t: string) => t.toLowerCase() === namaProfesi.toLowerCase()).length;`;
if (code.includes(tableBLogicStr)) {
  code = code.replace(tableBLogicStr, `const matchCount = (p.tags || []).filter((t: string) => {
